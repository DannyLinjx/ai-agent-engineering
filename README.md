...
把 Agent 当作一个**受治理的运行时**来设计、构建、重构、调试、扩展、测试、审计、文档化并交付生产化：模型在类型化契约内做决策，确定性代码负责授权、预算、执行、持久化和验证；最终交付的是可运行、可测试、带证据的增量系统，而不是一个巨大的 `agent.py`、一个只有提示词的安全方案，或一次性的单次 LLM 调用。

本 Skill 适用于创建或升级 Codex 类、OpenClaw 类、编码类、研究类、RAG 类、数据类、办公类、电脑控制类以及多 Agent 产品；也适用于把聊天机器人或单次 LLM 调用改造为安全、持久、可测试的 Agent。
